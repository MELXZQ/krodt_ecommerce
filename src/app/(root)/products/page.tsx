import { parseQuery, parseFilterParams } from "@/lib/utils/query";
...
export default async function ProductsPage(props: { searchParams: SearchParams }) {
  ...
  const filters = parseFilterParams(parsed);
  ...
  ["gender", "size", "color", "priceMin", "priceMax", "brand", "category"].forEach((k) => {
    const val = (parsed as Record<string, unknown>)[k];
    toArray(val).forEach((v) => activeBadges.push({ key: k, value: v }));
  });
